...
# Generate tree
pars <- data.frame(state=c(1,2), prior=c(0.5,0.5), lambda=c(3,1.5), mu=c(0.1,0.05), psi=c(0.1,1))
...
tryCatch({
    ...
    rho_1 <- 0.1  # sampling rate for state 1
...
for i in $(seq 1 $N_SIMS); do
    ...
    # TreeTime with custom sampling bias correction
    echo "Calculating custom correction factor for simulation $i..."
    custom_correction=$(Rscript -e "
        data <- read.csv('metadata_${i}.csv')
        t <- table(data\$numeric_state)
        fraction <- (1-((0.1/1.1)^2 + (1/1.1)^2))/(1-((t[1]/sum(t))^2 + (t[2]/sum(t))^2))
        cat(fraction)
    ")
    
    output_dir="treetime_results_20_0"
    mkdir -p "$output_dir"
    
    treetime_custom_start=$(date +%s)
    treetime mugration \
        --tree "tree_${i}.nwk" \
        --states "metadata_${i}.csv" \
        --attribute numeric_state \
        --confidence \
        --sampling-bias-correction $custom_correction \
        --outdir "$output_dir" \
        --verbose 0 > /dev/null 2>&1
    treetime_custom_success=$?
    treetime_custom_time=$(($(date +%s) - treetime_custom_start))
    ...
    echo "simulation,method,time_seconds,success" > "external_timing.csv"
    echo "$i,treetime,$treetime_time,$([[ $treetime_success -eq 0 ]] && echo TRUE || echo FALSE)" >> "external_timing.csv"
    echo "$i,treetime_20_0,$treetime_custom_time,$([[ $treetime_custom_success -eq 0 ]] && echo TRUE || echo FALSE),$custom_correction" >> "external_timing.csv"
    echo "$i,pastml,$pastml_time,$([[ $pastml_success -eq 0 ]] && echo TRUE || echo FALSE)" >> "external_timing.csv"
